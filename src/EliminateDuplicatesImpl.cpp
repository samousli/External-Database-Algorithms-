...
void eliminateDuplicatesImpl(char *infile, unsigned char field, block_t *buffer,
    ...
    *passes = new uint(0);
    ...
    block_t outputBlock = buffer[nmem_blocks-1];
    ...
    input.read((char*)buffer, (nmem_blocks-1) * sizeof (block_t)); // fill buffer from file
    ++(*nios);

     currentRecord = buffer[0].entries[buffer[0].dummy]; // set currentRecord

    read_count = nmem_blocks-1; // initialize read_count value
    for (uint i = read_count; i <= block_count; i += read_count) { // read blocks while reached buffer size
            ...
            if(i == block_count){
                break; // exit from loop without calculate buffer size and read new data
            }
            // Read into buffer
            read_count = (nmem_blocks-1) < block_count - i ? (nmem_blocks-1) : block_count - i; // number of blocks to read
...
void testRecordsForDuplicates(block_t *buffer, int read_count, record_t *currentRecord,
    ...
    for (int y = 0; y < read_count; y++) { // iterate buffer
        ...
        while(buffer[y].dummy < buffer[y].nreserved) { // while all records visited from block
            ...
            if (compare == -1 ) { // if records are not equal
