...
void hash_join (char *infile1, char *infile2, unsigned char field, block_t *buffer,
    ...
    if (nmem_blocks < 4) {
        cout << "Not enough memory, canceling hash join." << endl;
        return;
    }
 ...
 */
vector<hash_pair> partition(char *infile1, char *infile2, unsigned char field,
                            block_t *buffer, uint nmem_blocks, size_t file1_size,
                            size_t file2_size, uint *nios) {
    ...
    vector<string> parts1 = hash_partition(infile1, file1_size, field,
                                           nmem_blocks, buffer, nios, salt),
                            parts2 = hash_partition(infile2, file2_size, field,
                                     nmem_blocks, buffer, nios, salt);
    ...
    for (int i = parts1.size() - 1; i > -1; --i) {
        ...
        if (size1 > nmem_blocks - 1 || size2 > nmem_blocks - 1) {
            ...
            /*
                        cerr << "Removing: " << (*(files1.begin() + i)).first
            ...
            */
            ...
            //cerr << "Pair: " << files1[i].first << ", " << files2[i].first
...
vector<string> hash_partition(char *file, size_t file_size, unsigned char field,
    ...
    for(uint b = 0; b < bucket_size; ++b) {
        ...
        // Push filenames into the vector in an ascending order
