...
void merge_sort_driver(uint total, uint mem) {
    ...
    *ios = new uint(0);
    ...
    cout << "Sorted: " << boolalpha << sorted  << endl
         << "Segments: " << *sorted_segs << endl
         << "IO's: " << *ios << endl
         << "Passes: " << *passes << endl;

    // print_file_contents(output_file);
    //convert_to_txt_file(output_file, "mergesort.txt", total);
...
void merge_join_driver(uint total1, uint total2, uint mem) {
    ...
    uint *nios_m = new uint(0), *nres_m = new uint(0);
    ...
    merge_join  (input1, input2, 1, buffer, mem, output_m, nres_m, nios_m);

    cout << get_cpu_time() - time << " merge time" << endl;

    cout << "Merge Join IOs:\t" << *nios_m << "\tPairs: " << *nres_m << endl;
...
void join_driver(uint total1, uint total2, uint mem) {
    ...
    *nios_m = new uint(0), *nres_m = new uint(0);
    ...
    // Sort the hash output first, makes the comparison easier
    uint dummy;
    merge_sort(output_h, 1, buffer, mem, output_h, &dummy, &dummy, &dummy);
...
void create_test_file(char *filename, uint nblocks) {
    ...
    for (uint b = 0; b < nblocks; ++b) { // for each block
        ...
        for (int r = 0; r < MAX_RECORDS_PER_BLOCK; ++r) { // for each record
            ...
            // prepare a record
            record.recid = recid++;
            record.num = rand(MAX_NUM); //INT_MAX
...
void convert_to_txt_file(char *input, char *output, uint block_count) {
    block_count = file_block_count(input);
...
// Returns true if the given block is sorted
bool is_sorted(block_t b, unsigned char field) {
    record_t pr = b.entries[0], nr;
    record_comparator cmp(field, false);

    for (uint r = 1; r < b.nreserved; ++r) {
        nr = b.entries[r];
        if (!cmp(pr, nr))
            return false;
        pr = nr;
